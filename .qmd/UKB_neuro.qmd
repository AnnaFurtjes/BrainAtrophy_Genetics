...
### Visualise neuroimaging 

```{r, eval = F}
# read in UKB neuroimaging data 
UKB = fread(paste0(wd, "/UKB_CrossNeuroIDP.txt"))

# plot variables
library(cowplot)
library(ggplot2)

p1 = plot_hist(dat = UKB, var = "diff_stand")+
...
p2 = plot_hist(dat = UKB, var = "ratio_stand")+
...
p3 = plot_hist(dat = UKB, var = "resid_stand")+
          xlab("ICV-TBV\n(Standardised residual score)")+
          make_pretty()
# delete SD stats
p3$layers[[2]]=NULL
...
# annotate N
n = nrow(UKB)
p1=p1 + ylab(paste0("Count\n(total N = ", n, ")"))
p2=p2 + ylab(paste0("Count\n(total N = ", n, ")"))
p3=p3 + ylab(paste0("Count\n(total N = ", n, ")"))

# combine plots
plot = plot_grid(p1, p2, p3, nrow = 1, labels = c("A", "B", "C"), label_size = 6, rel_widths = c(1,1,1.5))
# save plot
ggsave(paste0(wd, "UKBDistributionsAllCross.jpg"), plot = plot, width = 12, height = 5, units = "cm", dpi = 300)

```

![Atrophy measures in the UKB at inital neuroimaging visit](.jpg)
...
```{r rawChange, eval = T, message=F, warning=F,fig.dim=c(10,10)}
